...
$order_query= mysqli_query($conn,"SELECT * FROM `orders` WHERE name='$name' And phone='$pno' AND email='$email' AND payment= '$pmethod' AND address='$address' AND  total_products='$total_products' AND total_price='$final_total'") or die('query failed');
    ...
    $message[]="restaurant is currently closed opens tomorrow  between 7am to 8pm  Thank you !";
 }
...
<div class="checkdiv">
   ...
   <div>
   <lable >Name:</lable> <input type="text" id="Uname"  pattern="[a-zA-Z'-'\s]*" title="special characters are not allowed"  name="Name" placeholder="Enter your name" value=" <?php echo $fetch_user['Name'] ?>">
   <lable >Email:</lable> <input type="email" name="Email" id="uemail"  value="<?php echo $fetch_user['Email_id'] ?>"   pattern="[^@\s]+@[^@\s]+\.[^@\s]+" title="Invalid email address"  placeholder="Enter your email">
   <lable >Contact:</lable><input type="tel" id="Uphone" name="Phoneno" placeholder="Enter phone number"  pattern="[6-9]{1}[0-9]{9}"  title="Phone number with 6-9 and remaing 9 digit with 0-9"  value="<?php echo $fetch_user['Phone'] ?>">
